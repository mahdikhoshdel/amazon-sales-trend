...
def compute_sales_trend(file_path, original_product_name):
    try:
        ...
        product_data = df[df["name"] == original_product_name][
            ["date", "discount_price"]
        ].copy()
        ...
        logger.error(
            f"Error in compute_sales_trend for {original_product_name}: {str(e)}"
        )
        raise
...
class DataProcessor:
    ...
    def get_sales_trend(self, abridged_product_name):
        try:
            ...
            logger.error(
                f"Error getting sales trend for {abridged_product_name}: {str(e)}"
            )
            raise
